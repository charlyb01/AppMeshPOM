...
public:
	...
	void AddMesh(const QString& name, const Mesh& mesh, const Vector& position = Vector::Null);
	void AddMesh(const QString& name, const MeshColor& mesh, const Vector& position = Vector::Null);
	void DeleteMesh(const QString& name);
	void ClearAll();

	void UpdateMesh(const QString& name, const Vector& position);
	...
	void SetCameraMode(bool perspective);
	void SetNearAndFarPlane(const Vector2& planes);
